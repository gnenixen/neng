 ...
 */
module engine.thirdparty.dbox.collision.b2distance;
...
struct b2DistanceProxy
{
    ...
    void Set(const(b2Shape) shape, int32 index)
    {
        switch (shape.GetType())
        {
            ...
            {
                const(b2CircleShape) circle = cast(const(b2CircleShape))(shape);
            ...
            {
                const(b2PolygonShape) polygon = cast(const(b2PolygonShape))(shape);
                m_vertices = polygon.m_vertices.ptr;
            ...
            {
                const(b2ChainShape) chain = cast(const(b2ChainShape))(shape);
            ...
            case b2Shape.e_edge:
            {
                const(b2EdgeShape) edge = cast(const(b2EdgeShape))(shape);
...
struct b2SimplexVertex
{
    ...
    b2Vec2 w;           // wB - wA
    float32 a = 0;          // barycentric coordinate for closest point
...
float32 b2Distance( b2Vec2 a, b2Vec2 b ) {
    import engine.thirdparty.dbox.common.b2math : dist = b2Distance;
    return dist( a, b );
}
